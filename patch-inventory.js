...
const tableStartIndex = code.indexOf(tableStart);
const tableEndIndex = code.indexOf(tableEndStr) + tableEndStr.length;

if (tableStartIndex !== -1 && tableEndIndex !== -1) {
  const replacement = `<div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
  ...
  code = code.substring(0, tableStartIndex) + replacement + code.substring(tableEndIndex);
  
  // Now add the sort dropdown next to statusFilter
  const filterString = `<select className="px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-black/5 outline-none transition-all text-sm md:text-base bg-white" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>
                      <option value="all">All Status</option>
                      <option value="low">Low Stock</option>
                      <option value="out">Out of Stock</option>
                    </select>`;
  
  const sortDropdown = `<select className="px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-black/5 outline-none transition-all text-sm md:text-base bg-white" value={sortConfig ? \`\${sortConfig.key}-\${sortConfig.direction}\` : ''} onChange={e => {
  ...
  code = code.replace(filterString, filterString + '\n                    ' + sortDropdown);
  
  fs.writeFileSync('app/(shell)/inventory/page.tsx', code, 'utf8');
} else {
  console.log("Could not find table tags!");
}
