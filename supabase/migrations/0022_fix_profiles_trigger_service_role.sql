-- Fix: allow service-role requests (Edge Functions using SERVICE_ROLE_KEY) to write profiles.role/store_id.
-- The previous check used request.jwt.claim.role which is not reliable for service-key calls.
...
BEGIN
  -- service_role (Edge Functions / admin tasks) bypasses user-level checks; it's trusted.
