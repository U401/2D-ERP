-- Fix: Global GCash Uniqueness, Idempotency Keys, and DB Locking for Inventory
-- 1. Add client_side_id to sales table for idempotency
-- 2. Update finalize_sale to check GCash reference code GLOBALLY
-- 3. Add FOR UPDATE locking to inventory deduction

-- 1. Add client_side_id to sales table
ALTER TABLE public.sales
  ADD COLUMN IF NOT EXISTS client_side_id TEXT;

-- Create a unique index on (store_id, client_side_id) to prevent duplicate sales from the same client
...
BEGIN
  ...
  IF p_payment_method = 'gcash' THEN
    ...
    -- This prevents reusing the same GCash transaction in different stores.
    IF EXISTS (
  ...
  -- Create sale record with store_id + payment method + GCash data + client_side_id
  ...
  LOOP
    ...
    LOOP
      ...
      FOR v_batch_record IN
        ...
        FOR UPDATE -- LOCK: Lock batches being consumed
      LOOP
        IF v_needed <= 0 THEN
          EXIT;
        END IF;
