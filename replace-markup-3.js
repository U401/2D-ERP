...
const splitToken = "{activeTab === 'stock' && !error && (";
...
if (parts.length > 1) {
    ...
    const newStockTab = `{activeTab === 'stock' && !error && (
      ...
      {showAddModal && <AddIngredientModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} />}
      {editingIngredient && <EditIngredientModal isOpen={!!editingIngredient} onClose={() => setEditingIngredient(null)} ingredient={editingIngredient} />}
      {removingStock && <RemoveStockModal isOpen={!!removingStock} onClose={() => setRemovingStock(null)} ingredientName={removingStock.name} currentStock={removingStock.current} unit={removingStock.unit} onConfirm={async (amount) => { try { const result = await adjustIngredientStock(removingStock.id, removingStock.current - amount); if (!result.success) throw new Error(result.error); setRemovingStock(null); checkUserAndLoadData(); } catch (err) { alert(err instanceof Error ? err.message : 'Failed to remove stock'); } }} />}
    </div>
  )
}
`;
    fs.writeFileSync(filePath, before + newStockTab, 'utf8');
    console.log("Success - replaced tail of file");
}
